...
public class PlayerLooker: MonoBehaviour
{
	...
	public static PlayerLooker playerLooker;
	...
	void Start() {
		if (playerLooker == null)
			playerLooker = this.gameObject.GetComponent<PlayerLooker>();
	...
	void Update() {
		if(GameController.gameController.EditMode || GameController.gameController.ExitMode || GameController.gameController.MenuMode) {
			LockCursor(false);
		} else {
			LockCursor(true);
			LookRotation();
